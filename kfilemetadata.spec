...
Source0  : https://download.kde.org/stable/frameworks/6.14/kfilemetadata-6.14.0.tar.xz
Source1  : https://download.kde.org/stable/frameworks/6.14/kfilemetadata-6.14.0.tar.xz.sig
...
Requires: kfilemetadata-bin = %{version}-%{release}
...
%package bin
Summary: bin components for the kfilemetadata package.
Group: Binaries
Requires: kfilemetadata-data = %{version}-%{release}
Requires: kfilemetadata-license = %{version}-%{release}

%description bin
bin components for the kfilemetadata package.
...
%setup -q -n kfilemetadata-6.14.0
cd %{_builddir}/kfilemetadata-6.14.0
pushd ..
cp -a kfilemetadata-6.14.0 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1747179894
mkdir -p clr-build
pushd clr-build
...
%files bin
%defattr(-,root,root,-)
/V3/usr/bin/kfilemetadata_dump6
/usr/bin/kfilemetadata_dump6
...
%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libKF6FileMetaData.so.6.14.0
...
/usr/lib64/libKF6FileMetaData.so.6.14.0
